...
# Find Busybox
# Priority: Bundled -> Magisk -> KSU -> System
if [ -x "$MODDIR/system/bin/busybox" ]; then
    BB="$MODDIR/system/bin/busybox"
...
# 1. Kill existing instance (Robust)
# Find pids using ps + grep to avoid pkill variations
PIDS=$($BB ps -o pid,args | $BB grep "httpd -p $PORT" | $BB grep -v grep | $BB awk '{print $1}')
if [ -n "$PIDS" ]; then
    echo "Stopping existing instance (PIDs: $PIDS)..."
    for pid in $PIDS; do
...
# 3. Start Server
# -p: Port, -h: Home (Root), -c: Config
echo "🚀 Starting Web Server with $BB..."
$BB httpd -p $PORT -h "$WEBROOT"

# Verify start
sleep 1
if $BB netstat -an | $BB grep -q ":$PORT "; then
