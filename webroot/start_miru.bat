...
if not exist "%~dp0scripts\web_stream.py" (
    ...
    :: Strategy: Try Local Copy (SD Card) -> Fallback to ADB Pull
    
    :: 1. Scripts
    if exist "%~dp0scripts" (
        echo [Miru] Copying scripts from storage...
        xcopy /E /I /Q /Y "%~dp0scripts" "!WORK_DIR!\scripts"
    ) else (
        echo [Miru] Pulling scripts via ADB...
        adb pull "!MOD_PATH!/scripts" "!WORK_DIR!\scripts"
    )

    :: 2. Webroot
    if exist "%~dp0webroot" (
        echo [Miru] Copying webroot from storage...
        xcopy /E /I /Q /Y "%~dp0webroot" "!WORK_DIR!\webroot"
    ) else (
        echo [Miru] Pulling webroot via ADB...
        adb pull "!MOD_PATH!/webroot" "!WORK_DIR!\webroot"
    )

    :: 3. System Binaries (Optional on PC, but good to have)
    if exist "%~dp0system\bin" (
        echo [Miru] Copying binaries from storage...
        xcopy /E /I /Q /Y "%~dp0system\bin" "!WORK_DIR!\system\bin"
    ) else (
        adb pull "!MOD_PATH!/system/bin" "!WORK_DIR!\system\bin" >nul 2>&1
    )

    :: 4. Tools (Heavy)
    if exist "%~dp0tools" (
        echo [Miru] Copying local tools from storage...
        xcopy /E /I /Q /Y "%~dp0tools" "!WORK_DIR!\tools"
    ) else (
        echo [Miru] Pulling tools from module (This may take a while)...
        adb pull "!MOD_PATH!/tools" "!WORK_DIR!\tools"
    )
...
:: 1.1 Python Setup
:: Check for bundled Python in tools/python
set "BUNDLED_PY=%~dp0tools\python\python.exe"
if exist "!BUNDLED_PY!" (
...
:: 1.2 ADB Setup
:: Add bundled ADB to PATH (Priority: Scrcpy bundle -> Generic ADB)
set "TOOLS_DIR=%~dp0tools\scrcpy\scrcpy-win64-v2.4"
if exist "%TOOLS_DIR%\adb.exe" (
    echo [Miru] Using bundled ADB (Scrcpy): %TOOLS_DIR%
    set "PATH=%TOOLS_DIR%;%PATH%"
) else (
    if exist "%~dp0tools\adb\adb.exe" (
        echo [Miru] Using bundled ADB (Generic)...
